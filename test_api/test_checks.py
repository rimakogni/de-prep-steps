...
class TestRunTestDecorator:
    ...
    def test_run_test_decorator_prints_success_message_if_tests_are_passing(
        self, capsys
    ):
        @run_test
        def func_to_test():
        ...
        expected_feedback = (
            f"{BOLD_GREEN}func_to_test(){NORMAL_GREEN}: Test passed ✅{DEFAULT}"
        )

        assert expected_feedback in captured.out

    @patch("checks.print")
    def test_run_test_decorator_prints_failure_message_if_assert_fails(
        self, mock_print
    ):
        ...
        feedback_msg = (
            f"{BOLD_RED}testing_function(){NORMAL_RED}: "
    ...
    @patch("checks.print")
    def test_run_test_decorator_prints_failure_message_if_other_exception_raised(
        self, mock_print
    ):
...
class TestSkipTestDecorator:
    ...
    def test_skip_test_decorator_prints_skip_message(self, capsys):
        ...
        expected_feedback = (
            f"{BOLD_YELLOW}func_to_test(){NORMAL_YELLOW}: Test skipped 🔇{DEFAULT}"
        )
