...
def translate_key(student, key_to_change, translation):
    english_student = student.copy()
    
    if key_to_change in english_student:
        english_student[translation] = english_student.pop(key_to_change)
    
    return english_student
    pass
...
def find_first_dentist(people):
    for dentist in people:
        if dentist['is_dentist']==True:
            return dentist
    else:
        return None
    pass
...
def tally_people_in_manchester(people):
   ...
   #         cont+=1
   # return cont
    return sum(1 for person in people if person.get('lives', {}).get('city') == 'Manchester')
    pass
...
def get_pug_owners(dogs):
    pug_owners_list=[]
    for pug_owners in dogs:
        if pug_owners.get('breed',{}) == 'Pug':
            pug_owners_list.append(pug_owners.get('owner'))
    return pug_owners_list
    pass
...
def pluralise_keys(dictionary):
    ...
    for key, value in dictionary.items():
        ...
        if isinstance(value, list):
            ...
            # True
            new_key = key if key.endswith('s') else key + 's'
        else:
            new_key = key
