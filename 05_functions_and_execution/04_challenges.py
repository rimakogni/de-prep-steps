...
def get_first_n_items(numbers,n):
...
def move_item_to_end(numeros,key):
    numeros.append(numeros.pop(key))
    move=numeros
    return move
...
def update_user_age(users_account):
    users_account["personal_details"]["age"]+=1
...
def check_infinitive(french_word):
    infinitive = french_word[-2:]
    return infinitive == 're'or infinitive == 'ir' or infinitive == 'er'
